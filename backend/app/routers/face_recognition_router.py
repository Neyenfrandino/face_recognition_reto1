...
@router.post("/post")
async def face_recognition_post(schema:Face_recognition_post_schema, file: UploadFile = File(...), camera_capture: UploadFile = File(...)):
    response = face_recognition_post_repo(schema, file, camera_capture)
    try:
        return response 
